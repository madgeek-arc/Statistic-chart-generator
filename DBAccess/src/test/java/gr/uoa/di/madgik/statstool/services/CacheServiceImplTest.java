...
public class CacheServiceImplTest {
    ...
    private void waitForUpdate() throws Exception {
        Field f = CacheServiceImpl.class.getDeclaredField("updating");
        f.setAccessible(true);
        AtomicBoolean updating = (AtomicBoolean) f.get(service);
        long deadline = System.currentTimeMillis() + 5000;
        while (updating.get() && System.currentTimeMillis() < deadline) {
            Thread.sleep(10);
        }
        assertFalse(updating.get(), "Update did not finish within 5 s");
    }
    ...
    @Test
    public void concurrentUpdate_secondCallIsNoOp() throws Exception {
        ...
        service.updateCache(null, 100, 60);
        assertTrue(started.await(2, TimeUnit.SECONDS), "First update must start");

        // Second call while first is running must not throw and must not start another thread
        assertDoesNotThrow(() -> service.updateCache(null, 100, 60));

        release.countDown();
        waitForUpdate();

        // getEntries called only once — second call was ignored
